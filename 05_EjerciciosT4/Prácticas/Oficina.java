...
public class Oficina {
    ...
    public Oficina(String n, int tipo) throws IllegalArgumentException {
        try {
            nombre = n;
            if (tipo <= 0 || tipo >= 3) {
                trabajadores = new ArrayList<Persona>();
    ...
    public double getSueldoMedio() throws Exception {
        double media = 0;
        if (this.getTrabajadores().isEmpty()) {
            Exception e = new Exception("No hay ningun trabajador"
                    + " en la empresa");
        } else {
            for (Persona p : this.trabajadores) {
                media += p.getSueldo();
            }
            media = media / this.trabajadores.size();
        }
